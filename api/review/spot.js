...
const { SpotReview } = require("../../models/review");
...
module.exports = {
  reviewCreate: async (req, res) => {
    try {
      ...
      const review = new SpotReview(req.body);
  ...
  reviewList: async (req, res) => {
    try {
      const reviews = await SpotReview.find({});
  ...
  reviewRead: async (req, res) => {
    ...
    try {
      const review = await SpotReview.findById(id);
  ...
  reviewUpdate: async (req, res) => {
    ...
    try {
      ...
      const review = await SpotReview.findByIdAndUpdate(id, req.body, {
        new: true,
        runValidators: true,
      });
  ...
  reviewDelete: async (req, res, next) => {
    ...
    try {
      const review = await SpotReview.findByIdAndDelete(id);
